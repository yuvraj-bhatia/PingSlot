// src/app/api/ai-engine/datasets/[datasetId]/fields/route.ts
...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    const mlRes = await fetch(
      `${ML_API_BASE}/api/datasets/${datasetId}/fields`,
    ...
    if (!mlRes.ok) {
      ...
      console.error(
        "ML /api/datasets/{id}/fields error:",
        mlRes.status,
        text || "<empty>",
      );
    ...
    console.error("Next /api/ai-engine/datasets/[id]/fields error:", err);
    return NextResponse.json(
